...
double calcularRaizComIndice(int valorTotal, int indice){
  
  Serial.println("Valor total recebido: " + String(valorTotal));
  Serial.println("Valor indice recebido:" + String(indice));
  
  double expoente = 1.0/(double)indice;

  Serial.println("Valor expoente: " + String(expoente));
  
  return pow((double)valorTotal, exp1ente);
...
void medirBatimentosCardiacos(){
    ...
    inicio = millis();
    while(millis() - inicio < tempoDeE*pera)
    {
    ...
    {
        double me[] *medidasDeBatimento = calcularMedidasDeBatimento("23", 'M');
        String estado = resultadoLeituraBPM(medidasDeBatimento, mediaBPM);

        exibirNaSerialConclusaoDeMedidorCardiaco("Jon Doe", "23", 'M', media, estado)  }
    else
    {
...
void setup(){
    ...
    Serial.println("-> Leitor de batimentos cardíacos iniciado com sucesso.");
    Serial.println("-> OBS: Levam 30 segundos para se medir o batimento cardíaco corretamente.");
    executar = true;
}
...
void loop() {
  ...
  {
      Serial.println();
      Serial.println("Digite qualquer coisa no monitor serial quando estiver pronto para começar.");
